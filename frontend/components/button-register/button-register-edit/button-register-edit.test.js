require('./../../../../test/testdom')('<html><body></body></html>'); // создадим ДОМ

import ReactTestUtils from 'react-dom/test-utils';

const React = require('react'),
    fs = require('fs'),
    path = require('path');

let btnClickResult = null;


const btnClick = jest.fn();

describe.only('component test, button', () => {

    const Button = require('./button-register-edit.jsx'),
        style = require('../button-register-styles');

    let component = ReactTestUtils.renderIntoDocument(<Button
        value="Edit"
        onClick={btnClick}>
    </Button>);

    let buttonEdit = component.refs['btnEdit'];
    let button = buttonEdit.refs['button'];

    it('should be define', () => {
        expect(component).toBeDefined();
    });

    it('should have property show, disabled (default', () => {
        expect(buttonEdit.props.show).toBe(true);
        expect(buttonEdit.props.disabled).toBe(false);
    });

    it('should exist children image', () => {
        let image = component.refs['image'];
        expect(image).toBeDefined();
    });

    it('should exist image file', () => {
        let image = component.refs['image'],
            publicPath = path.join(__dirname, '../../../../public/'),
            src = path.join(publicPath, style.icons.edit);


        let imageFile = fs.statSync(src);
        expect(imageFile.isFile()).toBe(true);

    });

    it('should be response from click event',() => {
        ReactTestUtils.Simulate.click(button);
        expect(btnClick).toBeCalled();

    })

});
